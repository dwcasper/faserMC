...
FaserCluster::FaserCluster(int index, vector<FaserDigi*>& digis)
    ...
    , fH(TString("cluster_"+std::to_string(index)),
         TString("cluster_"+std::to_string(index)),
         digis.size(), -0.5, digis.size() - 0.5)
    , fFitName("none")
{

    threeParGauss = make_shared<TF1>("g3", "gaus");
    fivePar2Gauss = make_shared<TF1>("g5", "[0]*exp(-0.5*((x-[1])/[2])^2)+[3]*exp(-0.5*((x-[4])/[2])^2)");
    sixPar2Gauss = make_shared<TF1>("g6", "[0]*exp(-0.5*((x-[1])/[2])^2)+[3]*exp(-0.5*((x-[4])/[5])^2)");
...
vector<FaserCluster::ClusterFit> FaserCluster::WeightedAverage() {
    // bail out on 1 hit cluster
    if (fDigis.size() == 1) {
        fFitResults.push_back(ClusterFit{0.0, fDigis[0]->Strip(), 0.5, 0.5, fDigis[0]->Charge(), 1});
        return fFitResults;
    }
    ...
    fFitResults.push_back(ClusterFit{fit->GetChisquare(), wtdX + fDigis[0]->Strip(), 
                errX, widX, fit->GetParameter(0), fDigis.size() - 1});
...
vector<FaserCluster::ClusterFit> FaserCluster::Fit() {
    ...
    if (fNMaxima == 1 && fDigis.size()>3) {
        ...
        for (int i = 0; i <= 2; i++) {
            ...
            if (fit->GetParameter(i) < pLow || fit->GetParameter(i) > pHigh) {
                cout << "Sanity check on 1gauss fit failed\n";
            }
        }

        fH.SetMaximum(max(fH.GetMaximum(),1.05*fit->GetParameter(0)));
        fFitResults.push_back(ClusterFit{fit->GetChisquare(), fit->GetParameter(1) + fDigis[0]->Strip(), 
                    fit->GetParError(1), fit->GetParameter(2), fit->GetParameter(0), fDigis.size() - 3});
    ...
    if (fDigis.size()>=5 && fNMaxima == 2) {
        ...
        //  fDigis[fMaxima[1]]->Charge() << " at " << fMaxima[1] << "\n";
        shared_ptr<TF1> theFunc = nullptr;
        if (fDigis.size() == 5) {
        ...
        for (int i = 0; i <= (fFitName == "g5" ? 4 : 5); i++) {
            ...
            if (fit->GetParameter(i) < pLow || fit->GetParameter(i) > pHigh) {
                cout << "Sanity check on 2gauss fit failed\n";
            }
        }
